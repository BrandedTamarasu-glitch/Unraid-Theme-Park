...
The beta targets Unraid `7.2+`. Manual validation is confirmed on Unraid `7.3.0` with `v0.1.7`, which is the current community test baseline. Latest `7.2.x` validation is still required before a wider Community Applications release.
...
- At least one successful install-from-URL report on Unraid `7.3.x`. Confirmed on Unraid `7.3.0` with `v0.1.7`.
